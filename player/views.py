	...
	if not request.user.is_authenticated():
		...
		if request.method == "POST":
			...
			if request.is_ajax():
				...
				if request.user not in users_who_have_listened:
					currentSng.listened.add(user)

				users_who_have_rated = []
				ratings_for_csong = Rating.objects.filter(song=currentSng).exclude(user=user)
				for rating in ratings_for_csong:
					if rating.user not in users_who_have_rated:
						users_who_have_rated.append(rating.user)
				#print(users_who_have_rated)

				all_possible_songs = []
				avg_user_ratings = []
				for usr in users_who_have_rated:
					ratings = Rating.objects.filter(user=usr)
					avg = 0
					for rate in ratings:
						pnsong = rate.song
						try:
							a = Rating.objects.get(song=pnsong, user=user)
							#print(pnsong, " lai chai rate gareko raichha")
						except:
							if pnsong not in all_possible_songs:
								if pnsong != currentSng:
									all_possible_songs.append(pnsong)
						finally:
							avg += rate.star
					avg_user_ratings.append(avg/len(ratings))

				#print("\n avg ratings ", avg_user_ratings )
				shuffle(all_possible_songs)
				#print(all_possible_songs)
				nextSng = {}
				previous_weight = -2.00
				similarity_weights = []
				if all_possible_songs == []:
					songs = currentSng.album.song_set.all()
					nextOne = songs[0]
					...
					for pnsong in all_possible_songs:
						numerator, denominator = 0, 1
						da,db=0,0
						for usr,avg_ratings in zip(users_who_have_rated,avg_user_ratings):
							crate = Rating.objects.get(song=currentSng, user=usr)
							csrating = crate.star
							try:
								nrate = Rating.objects.get(song=pnsong, user=usr)
								nsrating = nrate.star
							except:
								#print(usr, " has not rated ",pnsong)
								continue
							na,nb = csrating-avg_ratings,nsrating-avg_ratings
							numerator += na*nb
							da += na**2
							db += nb**2
						denominator = sqrt(da)*sqrt(db)
						weight = numerator/denominator
						similarity_weights.append((pnsong,weight))
